...
public class CreditAccount extends Account {
    ...
    public CreditAccount(int initialBalance, int creditLimit, int rate) {
        ...
        if (creditLimit < 0) {
            throw new IllegalArgumentException(
                    "Кредитный лимит не может быть отрицательным числом, а у вас: " + creditLimit
            );
        }
        if (initialBalance < -creditLimit) {
            throw new IllegalArgumentException(
                    "Начальный отрицательный баланс не может превысить кредитный лимит, а у вас: " + initialBalance + " > " + -creditLimit
            );
        }
